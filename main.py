...
async def user_balance(token: str):
    url = 'https://api-web.tomarket.ai/tomarket-game/v1/user/balance'
    headers = {'Authorization': token}
    async with aiohttp.ClientSession(headers=headers) as session:
        data = await request(session, 'POST', url)
        print_timestamp(
...
async def claim_game(token: str, points: int):
...
async def claim_daily(token: str):
...
async def start_farm(token: str):
...
async def claim_farm(token: str):
            ...
            for task in data['data'][category]:
                ...
                if not 'special' in task['tag']:
                    if task['status'] == 0 and task['type'] == "mysterious":
                        ...
                        await claim_tasks(token=token, task_id=task['taskId'])
                    elif task['status'] == 0:
                        print_timestamp(f"{Fore.YELLOW + Style.BRIGHT}[ Starting {task['title']} ]{Style.RESET_ALL}")
                        start_task = await start_tasks(token=token, task_id=task['taskId'])
                        if start_task['data']['status'] == 1:
                            ...
                            await check_tasks(token=token, task_id=task['taskId'])
...
async def start_tasks(token: str, task_id: int):
    url = 'https://api-web.tomarket.ai/tomarket-game/v1/tasks/start'
    headers = {'Authorization': token}
    payload = {'task_id': task_id}
    async with aiohttp.ClientSession(headers=headers) as session:
        return await request(session, 'POST', url, json=payload)

async def check_tasks(token: str, task_id: int):
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        ...
        if data['data']['status'] == 0:
            await start_tasks(token=token, task_id=task_id)
            ...
            await claim_tasks(token=token, task_id=task_id)

async def claim_tasks(token: str, task_id: int):
